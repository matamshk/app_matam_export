...
(function (root, factory) {
    ...
    class PrayerTimes {
        ...
        calculate() {
            ...
            // Apply static overrides if available in local settings
            try {
                if (typeof window !== 'undefined' && window.localStorage) {
                    const settingsStr = window.localStorage.getItem('site_settings');
                    if (settingsStr) {
                        const settings = JSON.parse(settingsStr);
                        if (settings && settings.prayer_calendar_database) {
                            const year = this.date.getFullYear();
                            const month = (this.date.getMonth() + 1).toString().padStart(2, '0');
                            const day = this.date.getDate().toString().padStart(2, '0');
                            const dateKey = `${year}-${month}-${day}`;
                            const staticData = settings.prayer_calendar_database[dateKey];
                            if (staticData) {
                                const parseTime = (timeStr, isPM) => {
                                ...
                                this.fajr = parseTime(staticData.fajr, false);
                                this.sunrise = parseTime(staticData.sunrise, false);
                                this.dhuhr = parseTime(staticData.dhuhr, false);
                                this.maghrib = parseTime(staticData.maghrib, true);
                                this.sunset = parseTime(staticData.sunset, true);
                                this.midnight = parseTime(staticData.midnight, true);
                ...
                console.warn("Failed to apply static prayer overrides:", err);
            }
        }
    ...
    function getHijriDate(date, hijriOffset) {
        try {
            if (typeof window !== 'undefined' && window.localStorage) {
                const settingsStr = window.localStorage.getItem('site_settings');
                if (settingsStr) {
                    ...
                    if (settings && settings.prayer_calendar_database) {
                        ...
                        if (staticData) {
                            return {
                                day: staticData.hijri_day,
                                month: staticData.hijri_month,
                                year: staticData.hijri_year,
                                monthName: "ذي الحجة",
                                isOverridden: true
                            };
                        }
        ...
        try {
            const hDate = new Date(date.getTime() + (hijriOffset || 0) * 86400000);
            const formatter = new Intl.DateTimeFormat('en-u-ca-islamic-umalqura', {
                day: 'numeric',
                month: 'numeric',
                year: 'numeric'
            });
            const parts = formatter.formatToParts(hDate);
